...
bool fexist(const std::filesystem::path& path) noexcept; //!< ファイルまたはディレクトリが存在すればtrue
...
void AddLastYenFromDirectoryPath(A& folder)
{
	// パスの末尾に円マークを付ける
	cxx::AddLastYenFromDirectoryPath(std::span<WCHAR>{ folder });
}
