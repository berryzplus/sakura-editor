...
int CDlgTypeList::DoModal( HINSTANCE hInstance, HWND hwndParent, SResult* psResult )
{
	...
	nRet = (int)CDialog::DoModal( hInstance, hwndParent, IDD_TYPELIST, (LPARAM)nullptr );
	if( -1 == nRet ){
		return FALSE;
	}
	else{
...
BOOL CDlgTypeList::OnLbnDblclk( int wID )
{
	...
	case IDC_LIST_TYPES:
		...
		::EndDialog(
			GetHwnd(),
			ApiWrap::List_GetCurSel( GetItemHwnd( IDC_LIST_TYPES ) )
...
BOOL CDlgTypeList::OnBnClicked( int wID )
{
	...
	//	Nov. 29, 2000	To Here
	case IDOK:
		::EndDialog( GetHwnd(), ApiWrap::List_GetCurSel( GetItemHwnd( IDC_LIST_TYPES ) ) );
		return TRUE;
	case IDCANCEL:
		::EndDialog( GetHwnd(), -1 );
		return TRUE;
...
INT_PTR	CDlgTypeList::DispatchDlgEvent(HWND hWndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		if( LOWORD(wParam) == IDC_LIST_TYPES )
		{
			...
			case LBN_SELCHANGE:
				...
				if( type->m_szTypeExts[0] == '\0' ){
					...
					ApiWrap::BtnCtl_SetCheck( hwndRMenu, m_bExtRMenu[ nIdx ] );
		...
		{
			bool checked = ( ApiWrap::BtnCtl_GetCheck( hwndRMenu ) != FALSE ? true : false );
		...
		{
			bool checked = ( ApiWrap::BtnCtl_GetCheck( hwndDblClick ) != FALSE ? true : false );
			if( ! AlertFileAssociation() ){		//レジストリ変更確認
				ApiWrap::BtnCtl_SetCheck( hwndDblClick, !checked );
				break;
			}
...
{
	int		nIdx;
	WCHAR	szText[64 + MAX_TYPES_EXTS + 10];
	int		nExtent = 0;
	HWND	hwndList = GetItemHwnd( IDC_LIST_TYPES );
	...
	if( -1 == selIdx ){
		selIdx = ApiWrap::List_GetCurSel( hwndList );
		if( -1 == selIdx ){
			selIdx = 0;
		}
	}
	...
	ApiWrap::List_ResetContent( hwndList );	/* リストを空にする */
	...
	ApiWrap::List_SetHorizontalExtent( hwndList, nExtent + 8 );
	if( GetDllShareData().m_nTypesCount <= selIdx ){
		selIdx = GetDllShareData().m_nTypesCount - 1;
	}
...
bool CDlgTypeList::InitializeType( void )
{
	...
	int			nRet;
	if( typeMini->m_szTypeExts[0] != L'\0' || iDocType == 0 ){ 
		nRet = ::MYMESSAGEBOX(
			GetHwnd(),
...
int CopyRegistry(HKEY srcRoot, const std::wstring& srcPath, HKEY destRoot, const std::wstring& destPath)
{
	...
	for (;;)
	{
		...
		if( errorCode == ERROR_NO_MORE_ITEMS ){
			...
			if( (errorCode = CopyRegistry(srcRoot, srcPath + L"\\" + szSubKey, destRoot, destPath + L"\\" + szSubKey)) ){ return errorCode; }
			index++;
		}
	}
...
int DeleteRegistry(HKEY root, const std::wstring& path)
{
	...
	for (;;)
	{
		...
		if( errorCode == ERROR_NO_MORE_ITEMS ){
			...
			if( (errorCode = DeleteRegistry(root, path + L"\\" + szSubKey)) != 0 ){ return errorCode; }
		}
	}
	keySrc.Close();
...
int RegistExt(LPCWSTR sExt, bool bDefProg)
{
	...
	if(wcscmp( sGenProgID.c_str(), szProgID ) != 0) {
		...
		if( (errorCode = keyExt.SetValue(nullptr, sGenProgID.c_str())) != 0 ){ return errorCode; }
	}
	...
	if( (errorCode = keyShellAction.SetValue(nullptr, L"Sakura &Editor")) != 0 ){ return errorCode; }
	...
	if(bDefProg)
	{
		if( wcscmp(szShellValue, ACTION_NAME) != 0 )
		{
			if( szShellValue[0] != '\0')
			{
				...
				keyBackup.SetValue(nullptr, szShellValue);
			}
			keyShell.SetValue(nullptr, ACTION_NAME);
		}
	}
	else
	{
