...
struct SLoadInfo
{
	...
	constexpr SLoadInfo(const A& _cFilePath, ECodeType _eCodeType, bool _bReadOnly, CTypeConfig _nType = CTypeConfig(-1))
...
struct SSaveInfo{
	...
	template <basis::NullTerminatedStringConstructible<WCHAR> A>
	constexpr SSaveInfo(const A& _cFilePath, ECodeType _eCodeType, const CEol& _cEol, bool _bBomExist)
		: cFilePath(_cFilePath)
		, eCharCode(_eCodeType)
