...
void CEditDoc::InitDoc()
{
	...
	GetEditWnd().SetGrepMode(false);	/* Grepモード */	//$$同上
...
) const
{
	...
	//GREPモード
	pfi->m_bIsGrep = CAppMode::getInstance()->IsGrepMode();
...
bool CEditDoc::IsAcceptLoad() const
{
	...
	if(CAppMode::getInstance()->IsGrepMode())return false;
...
)
{
	...
	CProgressSubject* pOld = GetEditWnd().m_pcVisualProgress->CProgressListener::Listen(&m_cLayoutMgr);
	...
	GetEditWnd().m_pcVisualProgress->CProgressListener::Listen(pOld);
	GetEditWnd().ClearViewCaretPosInfo();
...
BOOL CEditDoc::OnFileClose(bool bGrepNoConfirm)
{
	...
	if( CAppMode::getInstance()->IsGrepMode() ){
