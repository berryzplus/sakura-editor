...
{
	return DlgProc( reinterpret_cast<pDispatchPage>(&CPropKeybind::DispatchDlgEvent), hwndDlg, uMsg, wParam, lParam );
}
			...
			//case LBN_DBLCLK:
				{
					...
					nIndex = ApiWrap::List_GetCurSel( hwndAssignedkeyList );
					wmemset(buff, 0, int(std::size(buff)));
					ret = ApiWrap::List_GetText( hwndAssignedkeyList, nIndex, buff);
					if( ret != LB_ERR )
					{
						i = 0;
						p = buff;
						...
						for(j = 0; j < m_Common.m_sKeyBind.m_nKeyNameArrNum; j++)
						{
							...
							{
								ApiWrap::List_SetCurSel( hwndKeyList, j);
