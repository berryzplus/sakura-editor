...
{
	return DlgProc( reinterpret_cast<pDispatchPage>(&CPropGrep::DispatchDlgEvent), hwndDlg, uMsg, wParam, lParam );
}
...
void CPropGrep::SetRegexpVersion( HWND hwndDlg )
{
	...
	ApiWrap::DlgItem_GetText( hwndDlg, IDC_EDIT_REGEXPLIB, regexp_dll, int(std::size(regexp_dll)));
	CBregexp breg;
	if( DLL_SUCCESS != breg.InitDll( regexp_dll ) ){
		ApiWrap::DlgItem_SetText( hwndDlg, IDC_LABEL_REGEXP_VER, LS(STR_PROPCOMGREP_DLL) );
		return;
	}
	ApiWrap::DlgItem_SetText( hwndDlg, IDC_LABEL_REGEXP_VER, breg.GetVersionW() );
}
