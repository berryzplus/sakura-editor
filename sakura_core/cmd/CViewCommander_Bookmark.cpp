...
re_do:;								// hor
	...
	if(bFound){
		...
		AlertNotFound(false, LS(STR_BOOKMARK_NEXT_NOT_FOUND));
	}
	return;
}
...
re_do:;								// hor
	...
	if(bFound){
		...
		AlertNotFound(false, LS(STR_BOOKMARK_PREV_NOT_FOUND) );
	}
	return;
}
...
void CViewCommander::Command_FUNCLIST_NEXT(void)
{
	...
	m_pCommanderView->SendStatusMessage(LS(STR_ERR_SRNEXT2));
	AlertNotFound(false, LS(STR_FUCLIST_NEXT_NOT_FOUND));
	return;
}
...
void CViewCommander::Command_FUNCLIST_PREV(void)
{
	...
	m_pCommanderView->SendStatusMessage(LS(STR_ERR_SRPREV2));
	AlertNotFound(false, LS(STR_FUCLIST_PREV_NOT_FOUND) );
	return;
}
