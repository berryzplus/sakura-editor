 ...
 *
 * メニュー項目がトップメニューかどうかの判定に使う。
 * namespace nsFuncCodeには未定義。
 */
...
/*!
 * @brief ポップアップメニュー項目の機能ID
 *
 * メニュー項目がポップアップメニューかどうかの判定に使う。
 * namespace nsFuncCodeには未定義。
 */
 ...
 * @brief 特殊メニュー項目の機能ID
 *
 * メニュー項目が特殊メニューかどうかの判定に使う。
 * サクラエディタの特殊メニューは、動的なサブメニューを生成するもの。
...
struct SMenuItem {
	...
	int				m_nLevel = 1;			//!< メニュー階層。トップ、メニューバー項目なら0。
	char			m_chAccessKey = '\0';	//!< メニュー項目のアクセスキー。\0は「なし」。

	/*!
	 * @brief メインメニュー用コンストラクター
	 *
	 * メニューの階層構造を表現できる形式。
	 */
	constexpr SMenuItem(
	...
	/*!
	 * @brief カスタムメニュー用コンストラクター
	 *
	 * 階層構造のないポップアップメニューを表現する形式。
	 */
	constexpr explicit SMenuItem(
		EFunctionCode eFuncCode,
		char accessKey = '\0'
	) noexcept
		: m_eFuncCode(eFuncCode)
		, m_chAccessKey(accessKey)
	{
	}
...
EMainMenuType SMenuItem::GetType() const noexcept
{
	...
	// 特殊メニュー
	if (const auto found = specialFuncCodes.find(static_cast<int>(m_eFuncCode)); found != specialFuncCodes.cend()) return T_SPECIAL;

	// 最上位ポップアップメニュー
	if (const auto found = topFuncCodes.find(static_cast<int>(m_eFuncCode)); found != topFuncCodes.cend()) return T_NODE;

	// ポップアップメニュー
	if (const auto found = popupFuncCodes.find(static_cast<int>(m_eFuncCode)); found != popupFuncCodes.cend()) return T_NODE;
...
MATCHER(IsInitializedCommonSettingCustomMenu, "Checks if CommonSetting_CustomMenu is properly initialized") {
	...
	constexpr std::array editRMenu = {
		...
		SMenuItem{ F_COPY, 'C' },
		SMenuItem{ F_PASTE, 'P' },
		SMenuItem{ F_DELETE, 'D' },
		SMenuItem{ F_0 },
		SMenuItem{ F_COPY_CRLF, 'L' },
		SMenuItem{ F_COPY_ADDCRLF, 'H' },
		SMenuItem{ F_PASTEBOX, 'X' },
		SMenuItem{ F_0 },
		SMenuItem{ F_SELECTALL, 'A' },
		SMenuItem{ F_0 },
		SMenuItem{ F_TAGJUMP, 'G' },
		SMenuItem{ F_TAGJUMPBACK, 'B' },
		SMenuItem{ F_0 },
		SMenuItem{ F_COPYLINES, '@' },
		SMenuItem{ F_COPYLINESASPASSAGE, '.' },
		SMenuItem{ F_0 },
		SMenuItem{ F_COPYFNAME, 'F' },
		SMenuItem{ F_COPYPATH, '\\' },
		SMenuItem{ F_COPYDIRPATH, 'O' },
		SMenuItem{ F_0 },
		SMenuItem{ F_OPEN_FOLDER_IN_EXPLORER, 'E' },
		SMenuItem{ F_OPEN_COMMAND_PROMPT, 'W' },
	...
	constexpr std::array custMenu = {
		SMenuItem{ F_FILEOPEN, 'O' },
		SMenuItem{ F_FILESAVE, 'S' },
		SMenuItem{ F_NEXTWINDOW, 'N' },
		SMenuItem{ F_TOLOWER, 'L' },
		SMenuItem{ F_TOUPPER, 'U' },
		SMenuItem{ F_0 },
		SMenuItem{ F_WINCLOSE, 'C' },
	};

	constexpr std::array tabRMenu = {
		SMenuItem{ F_FILESAVE, 'S' },
		SMenuItem{ F_FILESAVEAS_DIALOG, 'A' },
		SMenuItem{ F_FILECLOSE, 'R' },
		SMenuItem{ F_FILECLOSE_OPEN, 'L' },
		SMenuItem{ F_WINCLOSE, 'C' },
		SMenuItem{ F_FILE_REOPEN, 'W' },
		SMenuItem{ F_0 },
		SMenuItem{ F_COPYFNAME, 'F' },
		SMenuItem{ F_COPYPATH },
		SMenuItem{ F_COPYDIRPATH },
		SMenuItem{ F_0 },
		SMenuItem{ F_OPEN_FOLDER_IN_EXPLORER, 'F' },
		SMenuItem{ F_OPEN_COMMAND_PROMPT, 'W' },
		SMenuItem{ F_OPEN_COMMAND_PROMPT_AS_ADMIN, 'w' },
		SMenuItem{ F_OPEN_POWERSHELL, 'P' },
		SMenuItem{ F_OPEN_POWERSHELL_AS_ADMIN, 'p' },
		...
		SMenuItem{ F_TAB_CLOSELEFT, 'H' },
		SMenuItem{ F_TAB_CLOSERIGHT, 'M' },
		SMenuItem{ F_0 },
		SMenuItem{ F_TAB_MOVERIGHT, '0' },
		SMenuItem{ F_TAB_MOVELEFT, '1' },
		SMenuItem{ F_TAB_SEPARATE, 'E' },
		SMenuItem{ F_TAB_JOINTNEXT, 'X' },
		SMenuItem{ F_TAB_JOINTPREV, 'V' },
	};
...
MATCHER(IsInitializedCommonSettingToolBar, "Checks if CommonSetting_ToolBar is properly initialized") {
	...
	constexpr std::array defaultTools = {
		1,		//F_FILENEW,			//新規作成
		25,		//F_FILEOPEN_DROPDOWN,	//ファイルを開く(DropDown)
		3,		//F_FILESAVE,			//上書き保存
	...
	for (size_t i = 0; i < std::size(defaultTools); ++i) {
...
MATCHER(IsInitializedCommonSettingSpecialKeyword, "Checks if CommonSetting_SpecialKeyword is properly initialized") {
	...
	const auto& cKeyWordSetMgr = sSpecialKeyword.m_CKeyWordSetMgr;

	EXPECT_THAT(cKeyWordSetMgr.m_nCurrentKeyWordSetIdx, 0);
...
#define CheckKeyword(name,case_sensitive) \
	...
	CheckKeyword(L"C/C++",			true);
	CheckKeyword(L"HTML",			false);
	CheckKeyword(L"PL/SQL",			false);
	CheckKeyword(L"COBOL",			true);
	CheckKeyword(L"Java",			true);
	CheckKeyword(L"CORBA IDL",		true);
	CheckKeyword(L"AWK",			true);
	CheckKeyword(L"MS-DOS batch",	false);
	CheckKeyword(L"Pascal",			false);
	CheckKeyword(L"TeX",			true);
	CheckKeyword(L"TeX2",			true);
	CheckKeyword(L"Perl",			true);
	CheckKeyword(L"Perl2",			true);
	CheckKeyword(L"Visual Basic",	false);
	CheckKeyword(L"Visual Basic2",	false);
	CheckKeyword(L"Rich Text",		true);
	CheckKeyword(L"Python",			true);
...
MATCHER(IsInitializedCommonSettingFileName, "Checks if CommonSetting_FileName is properly initialized") {
	...
	const std::array defaultConversions = {
		std::make_pair<std::wstring, std::wstring>( LR"(%DeskTop%\)",           LS(STR_TRANSNAME_DESKTOP) ),
		std::make_pair<std::wstring, std::wstring>( LR"(%Personal%\)",          LS(STR_TRANSNAME_MYDOC) ),
		std::make_pair<std::wstring, std::wstring>( LR"(%Cache%\Content.IE5\)", LS(STR_TRANSNAME_IE) ),
		std::make_pair<std::wstring, std::wstring>( LR"(%TEMP%\)",              LS(STR_TRANSNAME_TEMP) ),
		std::make_pair<std::wstring, std::wstring>( LR"(%Common DeskTop%\)",    LS(STR_TRANSNAME_COMDESKTOP) ),
		std::make_pair<std::wstring, std::wstring>( LR"(%Common Documents%\)",  LS(STR_TRANSNAME_COMDOC) ),
		std::make_pair<std::wstring, std::wstring>( LR"(%AppData%\)",           LS(STR_TRANSNAME_APPDATA) )
	};
...
MATCHER_P4(IsInitializedShareData, pszProfileName, isMultiUserSettings, userRootFolder, userSubFolder, "Checks if DLLSHAREDATA is properly initialized") {
	...
	auto& privateIniPath = iniPath; // CShareData::BuildPrivateIniFileName( iniPath, pszProfileName );
 ...
 * @brief 言語切替のテスト
 */
TEST_F(CShareDataTest, ChangeLang001)
{
	// 言語切り替えのテストを実施する
