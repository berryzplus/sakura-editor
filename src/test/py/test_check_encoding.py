...
def test_checkOriginMaster_success(mocker, check_encoding_module):
    """Test checkOriginMaster when origin/master check succeeds"""
    # Mock subprocess.check_output to simulate successful git command
    mock_check_output = mocker.patch("subprocess.check_output")
    mock_check_output.return_value = b"valid_output"
    
    # checkOriginMaster returns 0 on success (git returncode)
    result = check_encoding_module.checkOriginMaster()
    assert result == 0  # Success = 0


def test_checkOriginMaster_failure(mocker, check_encoding_module):
    """Test checkOriginMaster when origin/master check fails"""
    # Mock subprocess.check_output to raise CalledProcessError
    mock_check_output = mocker.patch("subprocess.check_output")
    mock_check_output.side_effect = subprocess.CalledProcessError(1, "git")
    
    # checkOriginMaster returns non-zero returncode on failure
    result = check_encoding_module.checkOriginMaster()
    assert result != 0  # Failure = non-zero


def test_getMergeBase_valid_commit(mocker, check_encoding_module):
    """Test getMergeBase returns valid commit hash"""
...
def test_getMergeBase_invalid_no_commit(mocker, check_encoding_module):
    """Test getMergeBase when no commit is found or error occurs"""
    # Mock subprocess.check_output to raise CalledProcessError
    mock_check_output = mocker.patch("subprocess.check_output")
    mock_check_output.side_effect = subprocess.CalledProcessError(1, "git")
    
    # getMergeBase will raise exception; test that it propagates
...
def test_getDiffFiles_generators_valid_files(mocker, check_encoding_module):
    """Test getDiffFiles generator yields files with target extensions"""
    # Mock getMergeBase to return a valid commit
...
def test_checkAll_generator_yields_target_extensions(mocker, check_encoding_module):
    """Test checkAll generator yields files with target extensions"""
...
def test_processFiles_detects_encoding_violations(tmp_path, check_encoding_module, capsys):
    """
    Test processFiles() integration: verify it detects encoding violations
    
    Setup:
    - Create a .cpp file with UTF-8 (valid)
    - Create a .rc file with UTF-8 (invalid, should be UTF-16)
    - Expect processFiles to detect the .rc as violation
    """
...
def test_processFiles_accepts_valid_files(tmp_path, check_encoding_module):
    """
    Test processFiles() with valid encodings
    
    Setup:
    - Create a .cpp file with UTF-8 (valid)
    - Create a .rc file with UTF-16 (valid)
    - Expect processFiles to return 0 violations
    """
    # Create valid .cpp file (UTF-8)
