...
def _destination_path(source: Path, destination: str) -> Path:
    """Resolve a file or trailing-separator directory destination."""
...
def copy_chm(source: Path, destination: str) -> bool:
    ...
    destination_path = _destination_path(source, destination)
...
def compile_project(
    ...
    *,
    ...
    destination_path = _destination_path(compiled_help, destination)
...
def main(arguments: Sequence[str] | None = None) -> int:
    ...
    if len(arguments) != 3:
        print(
            f"Usage: {Path(arguments[0]).name if arguments else 'compile_chm.py'} "
    ...
    return 0 if compile_project(project, arguments[2], os.environ) else 1
